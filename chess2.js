var canvas, ctx, size, size8, size16, imgs, chars, board, mouse, edit, flip, games, game;
...
window.onload = function(event) {
  ...
  canvas.onpointermove = function(event) {
    ...
    mouse.tx = Math.floor(mouse.x/size8) + 1;
    mouse.ty = bs.height - Math.floor(mouse.y/size8);
  ...
  function leftUp(event) {
    ...
    var x = Math.floor((mouse.x-dx)/size8) + 1,
        y = bs.height - Math.floor((mouse.y+dy)/size8);
...
function highlightMove(x,y,capture) {
  if (!capture || !(capture.x == x && capture.y == y)) {
    var cx = _cx((x - 1)*size8 + size16);
    var cy = _cy((bs.height - y)*size8 + size16);
    ctx.strokeStyle = 'black';
    ctx.lineWidth = size8*2/5;
    ctx.lineCap = 'round';
    ctx.globalAlpha = .25;
    ctx.beginPath();
    ctx.moveTo(cx,cy);
  ...
  if (capture) {
    var cx = _cx((capture.x - 1)*size8 + size16);
    var cy = _cy((bs.height - capture.y)*size8 + size16);
    ctx.strokeStyle = 'black';
    ctx.lineWidth = size8*4/50;
    ctx.globalAlpha = .25;
    ctx.beginPath();
    ctx.arc(cx, cy, size8*23/50, 0, 2*Math.PI);
    ctx.stroke();
    ctx.closePath();
    ctx.lineWidth = 1;
    ctx.globalAlpha = 1;
...
function drawArrow(arrow) {
  var x0 = (arrow.from.x - 1)*size8 + size16;
  var y0 = (bs.height - arrow.from.y)*size8 + size16;
  var x1 = (arrow.to.x - 1)*size8 + size16;
  var y1 = (bs.height - arrow.to.y)*size8 + size16;
  var t = size8*2/5;
  var w = size8/10;
...
function draw() {
  ...
  for(y=1;y<=bs.height;y++) {cy = (bs.height - y)*size8;
  for(x=1;x<=bs.width;x++) {cx = (x - 1)*size8;
    ...
    ctx.fillRect(cx,cy,size8,size8);
    ...
    ctx.drawImage(img,0,0,img.width,img.height,cx,cy,size8,size8);
  }}
  ...
  if (holding) {
    ...
    if (piece) {
      var img = imgs[piece.color][piece.type];
      ctx.drawImage(img,0,0,img.width,img.height,_cx(mouse.x-dx)-size16,_cy(mouse.y+dy)-size16,size8,size8);
    }
  }

  if (edit && sel.type != 'move' && sel.type != 'delete') {

    var cy = (bs.height - mouse.ty)*size8;
        cx = (mouse.tx - 1)*size8;
  ...
  if (q0) {
    if (Array.isArray(q0)) {
      q0.forEach(q0 => {
        ...
        if (piece) {
          var img = imgs[piece.color][piece.type];
          cx = (q0.current.x - 1)*size8;
          cy = (bs.height - q0.current.y)*size8;
          ctx.drawImage(img,0,0,img.width,img.height,cx,cy,size8,size8);
        }
      });
    } else {
